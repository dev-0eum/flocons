...
- `src/store/cardStore.ts` — zustand+persist. `cards: Record<wordId, CardState>`, `classify/reset`, key `flocons:cards:v1`, `version:1`+`migrate`+`partialize(cards)`. `isCorrect`(known=true)·`getCard`·`dueWordIds(now)` 헬퍼.
- `src/lib/storage.ts` — `asyncJSONStorage = createJSONStorage(() => AsyncStorage)` (스토어 공유).
- `app/learn.tsx` — 분류 4곳(swipe L/R·버튼)을 `handleClassify(value)`로 통합: `persistClassify(word.id, value, Date.now())`(영속) + `dispatch`(세션).
- **설계 대비**: `schedule`는 `prev: CardState`를 필수로 받고 "없으면 새 카드" 생성은 `cardStore.classify`가 `newCardState`로 처리(순수성 유지).
- **알려진 한계(설계 범위 밖)**: undo는 세션 덱만 되돌리고 영속 SRS는 되돌리지 않음 → 재분류 시 box 중복 승급 가능. 코드 주석에 명시, 필요 시 후속 Unit에서 처리.

## 3) 테스트 (qa-dev)
- [ ] `npm run typecheck` · [ ] `npm run lint` · [ ] `npm run test`(leitner·cardStore 라운드트립/마이그레이션) · [ ] `npx expo export`
...
- [ ] 게이트 4종 통과 · [ ] code-review 머지 가능 · 결정: <...>
### 커밋 / 푸시 (사용자 수행)
- 제안 메시지: `feat: SRS (Leitner) engine + persistent card store wired into learn deck [UoW-05]`
